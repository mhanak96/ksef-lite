...
import { AuthTokenRequestData } from "../types";
...
export async function signXmlSimple(
  ...
  try {
    ...
    console.log(`🔐 Detected key type: ${isECDSA ? "ECDSA" : "RSA"}`);
    console.log(`🔐 Using XAdES format (matching official KSeF C# implementation)`);
    ...
    console.log("📝 Certificate digest:", certDigest);
    ...
    console.log("📝 Document digest:", documentDigest);
    ...
    console.log("📝 Canonical SignedProperties:", canonicalSignedProperties.substring(0, 300));
    const signedPropertiesDigest = sha256Base64(canonicalSignedProperties);
    console.log("📝 SignedProperties digest:", signedPropertiesDigest);
    ...
    console.log("📝 Canonical SignedInfo length:", canonicalSignedInfo.length);
    ...
    console.log("✅ XML signed successfully (XAdES)");
    console.log("📝 Signed XML length:", signedXml.length);

    return signedXml;
  } catch (error) {
    console.error("🔴 Signing error:", error instanceof Error ? error.message : String(error));
